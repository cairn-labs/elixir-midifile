defmodule Midifile.Reader do
  ...
  defp read_event(
         ...
         pos,
         delta_time,
         <<@status_meta_event::size(8), type::size(8), _::size(8)>>,
         iter
       ) do
  ...
  defp read_event(f, pos, delta_time, <<@status_sysex::size(8), _::size(16)>>, iter) do
  ...
  defp read_event(f, pos, delta_time, <<b0::size(8), b1::size(8), _::size(8)>>, 0)
       when b0 < 128 do
    ...
    [event, num_bytes] =
      read_event(
        ...
        pos,
        delta_time,
        <<status::size(4), chan::size(4), b0::size(8), b1::size(8)>>,
        1
      )
  ...
  defp read_event(_f, _pos, delta_time, <<unknown::size(8), _::size(16)>>, iter) do
